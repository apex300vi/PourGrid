-- Correct Josh's physically verified BIB packages. Never infer Cranberry's ratio from package size.
update public.profit_lab_ingredients
set name='BIB Cranberry',
    catalog_product_name='BIB Cranberry',
    package_volume_oz=384,
    estimated_units_per_package=384,
    preparation_yield=1,
    syrup_to_water_ratio=null,
    mix_ratio_basis=null,
    mix_ratio_source=null,
    result_kind='estimated',
    serving_label='finished fl oz',
    assumption_note='Mix ratio required: Josh physically verified the 3 US gal concentrate package (384 fl oz), but the exact Cranberry product/SKU and dispensing ratio are not verified.',
    updated_at=now()
where key='cranberry-bib';

update public.profit_lab_ingredients
set name='BIB Minute Maid Lemonade 5 gal',
    catalog_product_name='Minute Maid Lemonade Bag in box, 5 Gallons',
    supplier_product_id='931422',
    package_volume_oz=640,
    estimated_units_per_package=640,
    preparation_yield=6,
    syrup_to_water_ratio=5,
    mix_ratio_basis='water_parts_per_one_syrup',
    mix_ratio_source='Minute Maid item 931422, GTIN 00049000988338, 1/5 gal; supplier product sheet states one gallon syrup yields 768 finished fl oz. https://sellsheets.syndigo.com/food/924d8a8e-2540-4997-806e-c08e1be64ae7?GTIN=00049000988338&ItemID=931422',
    result_kind='exact',
    serving_label='finished fl oz',
    assumption_note='Verified 5 US gal concentrate package (640 fl oz), 5 water parts per 1 concentrate part, 3,840 finished fl oz.',
    updated_at=now()
where key='lemonade-bib';
...
begin
 ...
 update public.profit_lab_ingredients set name='BIB Cranberry',catalog_product_name='BIB Cranberry',package_volume_oz=384,estimated_units_per_package=384,preparation_yield=1,syrup_to_water_ratio=null,mix_ratio_basis=null,mix_ratio_source=null,result_kind='estimated',serving_label='finished fl oz',assumption_note='Mix ratio required: Josh physically verified the 3 US gal concentrate package (384 fl oz), but the exact Cranberry product/SKU and dispensing ratio are not verified.',updated_at=now() where location_id=new.id and key='cranberry-bib';
