...
-- A retry must identify the conflict it already created. Older conflict rows did
-- not retain the mutation key, so reopening a device could insert the same
-- conflict again indefinitely.
alter table public.shared_draft_conflicts
  add column if not exists idempotency_key uuid;

comment on column public.shared_draft_conflicts.idempotency_key is
  'Stable client mutation key used to return one conflict for repeated delivery.';

-- These rows never represented a choice: both devices submitted the exact same
-- JSON value. Closing them changes no draft field, revision, count, or order.
update public.shared_draft_conflicts
set resolved_at=now(),resolved_by=created_by
where resolved_at is null
  and server_value=incoming_value;

create unique index if not exists shared_draft_conflicts_mutation_once
  on public.shared_draft_conflicts(draft_id,idempotency_key)
...
begin
  ...
  if prior_conflict.id is not null then
    ...
    if field_exists and field.value=p_value then
      update public.shared_draft_conflicts
  ...
  -- Revision drift is harmless when the value is already exactly what this
  -- device sent. Acknowledge it without changing the draft or asking a person.
  if field_exists and field.value=p_value then
    update public.shared_draft_conflicts
    set resolved_at=now(),resolved_by=actor
