-- Forward-only deployment repair for production, where 202608250001 was recorded without its body.
-- Statements intentionally match the reviewed idempotent migration.
...
set syrup_to_water_ratio=5,
    ...
    result_kind='exact', category='Fountain beverages',
    ...
    assumption_note='5 US gal syrup (640 fl oz), 5 water parts per 1 syrup part, 3,840 finished fl oz. Water and CO2 allocation excluded.', updated_at=now()
where key in('coke-bib','diet-coke-bib','fanta-orange-bib','sprite-bib');

update public.profit_lab_ingredients set syrup_to_water_ratio=null,mix_ratio_basis=null,mix_ratio_source='Official Coca-Cola dispenser manual requires the product BIB label for uncarbonated juice/mixer ratio. https://www.coca-colaparts.com/?fid=648&mod=parts&op=dwnattach',result_kind='estimated',category='Fountain beverages',catalog_product_name='BIB Cranberry',assumption_note='Mix ratio required: verified 3 US gal concentrate package (384 fl oz), but no authoritative product-label ratio is available.',updated_at=now() where key='cranberry-bib';
...
begin
 ...
 (new.organization_id,new.id,'club-soda-gun','BIB Club Soda','preset',null,0,1,1,1,0,'exact','finished fl oz','Virtual non-orderable gun ingredient: no syrup BIB or inventory demand.',array['SBB']::text[],'non_alcoholic',1,'oz','Merchants','PourGrid business rule: water and CO2 allocation excluded.',true) on conflict(location_id,key) do nothing;
 return new;
end$$;
