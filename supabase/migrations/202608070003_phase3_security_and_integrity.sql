...
begin
  if tg_table_name='order_lines' and exists(select 1 from public.structured_orders o where o.id=old.order_id and o.status<>'draft') then raise exception 'Submitted order facts are immutable'; end if;
