...
begin
 ...
 select jsonb_build_object('id',draft.id,'workspace',draft.workspace,'state',draft.state,'revision',draft.revision,'reviewedRevision',draft.reviewed_revision,'seasonalProfile',draft.seasonal_snapshot,'workflowState',draft.workflow_state,'updatedAt',draft.updated_at,'updatedBy',draft.updated_by,'fields',coalesce(jsonb_agg(jsonb_build_object('productKey',f.product_key,'fieldKey',f.field_key,'value',f.value,'revision',f.revision,'updatedAt',f.updated_at,'updatedBy',f.updated_by) order by f.revision) filter(where f.draft_id is not null),'[]'::jsonb),'conflicts',coalesce((select jsonb_agg(jsonb_build_object('id',c.id,'productKey',c.product_key,'fieldKey',c.field_key,'serverValue',c.server_value,'incomingValue',c.incoming_value,'serverRevision',c.server_revision,'expectedRevision',c.expected_revision,'createdAt',c.created_at) order by c.created_at) from public.shared_draft_conflicts c where c.draft_id=draft.id and c.resolved_at is null),'[]'::jsonb)) into result from public.shared_draft_fields f where f.draft_id=draft.id group by draft.id,draft.workspace,draft.state,draft.revision,draft.reviewed_revision,draft.seasonal_snapshot,draft.workflow_state,draft.updated_at,draft.updated_by;
 return result;
end$$;
