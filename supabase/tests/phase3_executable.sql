...
reset role;
-- Transaction-wrapped Profit Lab garnish fixtures: assumptions are shared, location scoped, and non-operational.
insert into public.profit_lab_ingredients(organization_id,location_id,key,name,source,catalog_product_name,packages_per_case,estimated_units_per_package,preparation_yield,waste_percent,result_kind,serving_label,assumption_note) values
 ('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','lime-wedge','Lime wedge','preset','Limes',1,200,8,0,'estimated','wedge','fixture'),
 ('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','lemon-wedge','Lemon wedge','preset','Lemons',1,200,8,0,'estimated','wedge','fixture'),
 ('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','maraschino-cherry','Maraschino cherry','preset','Maraschino Cherries',4,400,1,0,'estimated','cherry','fixture');
select test.assert((select count(*)=3 from public.profit_lab_ingredients where location_id='20000000-0000-0000-0000-000000000001' and source='preset'),'three garnish presets seeded per location');
select test.assert((select packages_per_case=4 and preparation_yield=1 and result_kind='estimated' from public.profit_lab_ingredients where location_id='20000000-0000-0000-0000-000000000001' and key='maraschino-cherry'),'cherry packaging and confidence retained');
set role authenticated; select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000006',false);
select public.save_profit_lab_ingredient('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001',jsonb_build_object('id',(select id from public.profit_lab_ingredients where location_id='20000000-0000-0000-0000-000000000001' and key='lime-wedge'),'packageCasePrice','80','packagesPerCase','1','estimatedUnitsPerPackage','200','preparationYield','8','wastePercent','20','resultKind','estimated','assumptionNote','fixture'));
