...
select public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','bar-fixture-draft','{"draftId":"bar-fixture-draft","orderType":"bar","counts":{"Vodka":"7"},"items":[{"name":"Vodka","calculatedOrderQty":1,"manualAdjustment":0,"finalOrderQty":1}]}') as saved_legacy_order_id \gset
select test.assert(public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','bar-fixture-draft','{"draftId":"bar-fixture-draft","orderType":"bar","counts":{"Vodka":"7"},"items":[{"name":"Vodka","calculatedOrderQty":1,"manualAdjustment":0,"finalOrderQty":1}]}')=:'saved_legacy_order_id','duplicate order save returns original id');
reset role;
select test.assert((select count(*)=1 from public.legacy_order_submissions where draft_id='bar-fixture-draft'),'duplicate retry creates one submission');
select test.assert((select organization_id='10000000-0000-0000-0000-000000000001' and location_id='20000000-0000-0000-0000-000000000001' from public.legacy_order_references where legacy_order_id=:'saved_legacy_order_id'),'new save receives exact organization and location attribution');
select test.assert((select md5(data::text)=s.payload_hash from public.orders o join public.legacy_order_submissions s on s.order_id=o.id where o.id=:'saved_legacy_order_id'),'identical retry preserves payload hash');
set role authenticated; select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000006',false);
select test.assert((select count(*)=1 from public.get_location_order_history('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001',60) where id=:'saved_legacy_order_id'),'saved order appears immediately in History');
do $$begin begin perform public.save_location_order('10000000-0000-0000-0000-000000000002','20000000-0000-0000-0000-000000000002','cross-tenant','{"draftId":"cross-tenant","orderType":"bar","counts":{},"items":[{"name":"Vodka"}]}'); raise exception 'cross-tenant order save unexpectedly succeeded'; exception when insufficient_privilege then null; end; begin perform public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000003','wrong-location','{"draftId":"wrong-location","orderType":"bar","counts":{},"items":[{"name":"Vodka"}]}'); raise exception 'wrong-location order save unexpectedly succeeded'; exception when insufficient_privilege then null; end; begin perform public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','bar-fixture-draft','{"draftId":"bar-fixture-draft","orderType":"bar","counts":{"Vodka":"8"},"items":[{"name":"Vodka"}]}'); raise exception 'changed duplicate unexpectedly succeeded'; exception when unique_violation then null; end; end$$;
do $$begin begin perform public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','history-hidden-type','{"draftId":"history-hidden-type","orderType":"bar","type":"counts","counts":{},"items":[{"name":"Vodka"}]}'); raise exception 'History-hidden order type unexpectedly succeeded'; exception when invalid_parameter_value then null; end; end$$;
reset role;
insert into auth.users(id,email) values('00000000-0000-0000-0000-000000000007','no-membership@example.com');
insert into public.profiles(id,display_name) values('00000000-0000-0000-0000-000000000007','no membership');
set role authenticated; select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000007',false);
do $$begin begin perform public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','no-membership','{"draftId":"no-membership","orderType":"bar","counts":{},"items":[{"name":"Vodka"}]}'); raise exception 'missing-membership order save unexpectedly succeeded'; exception when insufficient_privilege then null; end; end$$;
reset role;

create function test.reject_attribution_fixture() returns trigger language plpgsql as $$begin if exists(select 1 from public.orders o where o.id=new.legacy_order_id and o.data->>'draftId'='rollback-fixture') then raise exception using errcode='23514',message='fixture attribution failure'; end if; return new; end$$;
create trigger reject_attribution_fixture before insert on public.legacy_order_references for each row execute function test.reject_attribution_fixture();
set role authenticated; select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000006',false);
do $$begin begin perform public.save_location_order('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','rollback-fixture','{"draftId":"rollback-fixture","orderType":"bar","counts":{},"items":[{"name":"Vodka"}]}'); raise exception 'attribution failure unexpectedly committed'; exception when check_violation then null; end; end$$;
reset role;
drop trigger reject_attribution_fixture on public.legacy_order_references;
drop function test.reject_attribution_fixture();
select test.assert(not exists(select 1 from public.orders where data->>'draftId'='rollback-fixture'),'attribution failure rolls back order payload');
select test.assert(not exists(select 1 from public.legacy_order_submissions where draft_id='rollback-fixture'),'attribution failure rolls back submission');
select test.assert((select count(*)=33 from public.legacy_order_references where location_id is null),'new save does not alter existing null-reference audit population');
select test.assert((select count(*)=1 from public.legacy_order_references where legacy_order_id=:'saved_legacy_order_id' and location_id='20000000-0000-0000-0000-000000000001'),'existing correctly attributed reference remains unchanged');
set role authenticated; select set_config('request.jwt.claim.sub','00000000-0000-0000-0000-000000000006',false);
...
select test.assert((select (result->>'status')='acknowledged' and (result->>'noChange')::boolean from (select public.update_shared_draft_field('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001',:'noop_draft_id','Lime Juice','loose','"6"'::jsonb,0,'91000000-0000-0000-0000-000000000004') result)q),'numeric string and number acknowledge as the same count');
select test.assert((select jsonb_array_length(result->'conflicts')=0 from (select public.read_shared_location_draft('10000000-0000-0000-0000-000000000001','20000000-0000-0000-0000-000000000001','bar') result)q),'numeric representation difference creates no review choice');
