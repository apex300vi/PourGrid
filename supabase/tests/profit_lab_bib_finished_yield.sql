...
begin
 ...
 if exists(select 1 from public.profit_lab_ingredients where key='cranberry-bib' and (catalog_product_name<>'Harvest Valley 19% Cranberry Juice Cocktail' or supplier_product_id<>'758477' or package_case_price<>57.80 or package_volume_oz<>384 or estimated_units_per_package<>384 or preparation_yield<>5 or syrup_to_water_ratio<>4 or mix_ratio_basis<>'water_parts_per_one_syrup' or abs(public.profit_lab_bib_cost_per_finished_oz(package_case_price,package_volume_oz,syrup_to_water_ratio)-(57.80/1920))>0.000000001)) then raise exception 'verified Harvest Valley Cranberry metadata or cost is incorrect'; end if;
