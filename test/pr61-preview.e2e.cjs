...
(async()=>{
  ...
  let attemptedSaveBody=null;
  page.on('request',request=>{if(request.url().includes('/rpc/save_location_order'))attemptedSaveBody=request.postDataJSON()});
  await page.evaluate(async()=>{try{await window.POURGRID_ORDER_API.save({draftId:'pr61-invalid-attribution-probe',orderType:'bar',counts:{},items:[]})}catch(_error){}});
  const result=await page.evaluate(async({org,location})=>{
    ...
    return {context,historyRows:rows.length,normal:normal&&normal.name,multiplier:Number(normal&&normal.percentage_multiplier),crossTenantAllowed:response.ok};
  ...
  assert.equal(result.crossTenantAllowed,false,'isolated identity must not read target tenant');
  assert.ok(attemptedSaveBody,'deployed save adapter must call the save RPC');
  assert.equal(attemptedSaveBody.p_organization,result.context.organizationId);
  assert.equal(attemptedSaveBody.p_location,result.context.locationId);
